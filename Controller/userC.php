<?php
include_once 'c:/xampp/htdocs/projetuser/config.php';
include_once 'c:/xampp/htdocs/projetuser/Model/user.php';
...
class userC
{
    ...
    public function addUser(User $user) {
        try {
            ...
            // L'e-mail est unique, ajoutez l'utilisateur à la base de données avec prenom, adresse, téléphone
            $sql = "INSERT INTO user (nom, prenom, email, mdp, role, adresse, telephone) VALUES (?, ?, ?, ?, 0, ?, ?)";
            $stmt = $pdo->prepare($sql);
            $stmt->execute([$user->getNom(), $user->getPrenom(), $user->getEmail(), $user->getMdp(), $user->getAdresse(), $user->getTelephone()]);
    ...
    public function updateUser($userId, User $user) {
        try {
            $sql = "UPDATE user SET nom = :nom, prenom = :prenom, email = :email, mdp = :mdp, role = :role, adresse = :adresse, telephone = :telephone WHERE id = :id";
            $stmt = config::getConnexion()->prepare($sql);
            $stmt->bindParam(':id', $userId, PDO::PARAM_INT);
            $stmt->bindParam(':nom', $user->getNom(), PDO::PARAM_STR);
            $stmt->bindParam(':prenom', $user->getPrenom(), PDO::PARAM_STR);
            $stmt->bindParam(':email', $user->getEmail(), PDO::PARAM_STR);
            $stmt->bindParam(':mdp', $user->getMdp(), PDO::PARAM_STR);
            $stmt->bindParam(':role', $user->getRole(), PDO::PARAM_INT);
            $stmt->bindParam(':adresse', $user->getAdresse(), PDO::PARAM_STR);
            $stmt->bindParam(':telephone', $user->getTelephone(), PDO::PARAM_STR);
            $stmt->execute();
        } catch (PDOException $e) {
            die("Error: " . $e->getMessage());
        }
        ...
        try {
            ...
            if ($row) {
                return new User(
                    ...
                    $row['telephone']  // Récupération du telephone
                );
            } else {
                return null;
            }
