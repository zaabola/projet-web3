...
// Création de l'instance du contrôleur UserC
$userC = new UserC();

// Vérifie si l'id est passé en paramètre dans l'URL
if (isset($_GET['id'])) {
    $userId = $_GET['id'];
    ...
    // Si l'utilisateur n'est pas trouvé, redirection vers une page d'erreur
    if (!$user) {
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
  ...
  $prenom = $_POST["prenom"];
  $email = $_POST["email"];
  $mdp = $_POST["mdp"];  // Mot de passe envoyé via le formulaire (en clair)
  $role = $_POST["role"];
  $adresse = $_POST["adresse"];
  $telephone = $_POST["telephone"];
  $nationalite = $_POST["nationalite"];  // Récupère la nationalité

  try {
      // Créez un nouvel objet User avec les informations mises à jour
      $user = new User($userId, $nom, $prenom, $email, $mdp, $role, $adresse, $telephone, $nationalite);
    ...
    // Si l'id n'est pas passé dans l'URL, redirection vers la page d'index
        ...
        <form id="editForm" method="POST">
            ...
            <div class="form-group">
                ...
                <select id="nationalite" name="nationalite">
                    <option value="Tunisienne" <?php echo $user->getNationalite() == 'Tunisienne' ? 'selected' : ''; ?>>Tunisienne</option>
                    <option value="Etrangère" <?php echo $user->getNationalite() == 'Etrangère' ? 'selected' : ''; ?>>Etrangère</option>
                    <!-- Vous pouvez ajouter d'autres nationalités ici -->
                </select>
                <div id="nationaliteError" class="error-message"></div>
            </div>
